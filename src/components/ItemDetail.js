  ...
  if(item.length > 0) {
    return(
      <div className="md:flex shadow-lg  mx-6 md:mx-auto my-40 max-w-lg md:max-w-2xl h-2/3">
        <img className="h-full w-full md:w-1/3  object-cover rounded-lg rounded-r-none pb-5/6" src={item[0].pictureUrl} alt={`Producto: ${item[0].title}`} />
        <div className="w-full md:w-2/3 px-4 py-4 bg-white rounded-lg">
          <div className="flex items-center">
            <h2 className="text-xl text-gray-800 font-medium mr-auto">{item[0].title}</h2>
            <div className="flex justify-center flex-col p-1">
              <p className="text-right text-gray-500 font-semibold tracking-tighter">
                apurate!!
              </p>  
              <span className="text-gray-800 font-medium">{`últimas ${item[0].initialStock}`}</span>
            </div>  
          </div>
          ...
          <div className="flex items-center justify-between mt-2 top-auto">
            <div className="badge m-4 flex justify-end align-baseline">
              <div>
                  ...
                  <span className="font-semibold">{item[0].price}</span>
              </div>
              <div className="ml-2">
                  ...
                  <span className="font-semibold">{item[0].price*1.2}</span>
              </div>
            </div>  
            <div>
              <ItemCount initialStock={item[0].initialStock}/>
            </div>  
          </div>
        </div>  
      </div>
